...
## Sentinel — マルチエージェントオーケストレーター

ZeroClaw には **Sentinel** が組み込まれています。複雑な目標を達成するために、専門化された AI エージェントを統率するマルチエージェントオーケストレーターです。
...
| コードネーム | 役割 | 専門 |
|-------------|------|------|
| **Sentinel** | オーケストレーター | 目標分解、エージェント割当、結果統合 |
| **Archon** | システムアーキテクト | 設計、API 契約、アーキテクチャ判断 |
| **Prism** | UI/UX エキスパート | ユーザーフロー、コンポーネント、デザインシステム |
| **Forge** | フルスタックエンジニア | 実装、テスト、デバッグ、デプロイ |
| **Nexus** | ビジネス開発 | パートナーシップ、提案、戦略 |
| **Echo** | マーケティング | コンテンツ、キャンペーン、分析 |
| **Closer** | セールス | アウトリーチ、クオリフィケーション、デモ |
| **Oracle** | リサーチャー | 市場分析、競合調査、技術スカウティング |
| **Veasna** (វាសនា) | クメール語エキスパート | 文化翻訳、クメール語作成、ローカライゼーション |

### 仕組み

1. **ユーザーが目標を送信** → Sentinel が分類・分解
2. **Sentinel がサブタスクを割当** → 専門性に応じてエージェントにルーティング
3. **エージェントが並列実行** → Builder、Business、Research クラスタが同時に稼働
4. **プッシュ型完了通知** → 完了時にエージェントが結果を報告（ポーリング不要）
5. **Sentinel が統合** → 各出力を一つの成果物にまとめる

### 主要サブシステム

| サブシステム | パス | 機能 |
|-------------|------|------|
| オーケストレーター | `src/orchestrator/` | サブエージェント管理、スポーンポリシー、深度制限、完了通知 |
| コンテキストエンジン | `src/context/` | トークンバジェット、優先度ベースの組み立て、自動コンパクション |
| キャンバス | `src/canvas/` | Gateway 経由の対話型 UI（ダッシュボード、フォーム、レポート） |
| ペルソナ | `src/personas/` | 各エージェントのシステムプロンプト、ツール権限、モデル設定 |
| ワークスペース | `src/workspace/` | ユーザーごとの隔離ストレージ、パス検証、マルチユーザー管理 |
| デリゲートツール | `src/tools/delegate.rs` | Sentinel とサブエージェント間のブリッジ |
| Claude Code | `src/tools/claude_code.rs` | CLI 経由の計画/実装/レビュー/修正 |

詳細は英語版メインドキュメントをご覧ください: [`README.md`](README.md)
