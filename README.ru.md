...
## Sentinel — мульти-агентный оркестратор

ZeroClaw включает **Sentinel** — мульти-агентный оркестратор, координирующий специализированных AI-агентов для достижения сложных целей.
...
| Позывной | Роль | Специализация |
|----------|------|---------------|
| **Sentinel** | Оркестратор | Декомпозиция целей, назначение агентов, синтез результатов |
| **Archon** | Системный архитектор | Проектирование, API-контракты, архитектурные решения |
| **Prism** | UI/UX эксперт | Пользовательские потоки, компоненты, дизайн-системы |
| **Forge** | Fullstack-инженер | Реализация, тесты, отладка, деплой |
| **Nexus** | Бизнес-развитие | Партнёрства, предложения, стратегия |
| **Echo** | Маркетинг | Контент, кампании, аналитика |
| **Closer** | Продажи | Аутрич, квалификация, демонстрации |
| **Oracle** | Исследователь | Анализ рынка, конкурентная разведка, технический скаутинг |
| **Veasna** (វាសនា) | Кхмерский эксперт | Культурный перевод, кхмерский контент, локализация |

### Как это работает

1. **Пользователь отправляет цель** → Sentinel классифицирует и декомпозирует
2. **Sentinel назначает подзадачи** → маршрутизация по экспертизе
3. **Агенты работают параллельно** → кластеры Builder, Business, Research одновременно
4. **Push-уведомления о завершении** → агенты сами сообщают о результатах (без поллинга)
5. **Sentinel синтезирует** → объединяет результаты в единый deliverable

### Ключевые подсистемы

| Подсистема | Путь | Назначение |
|-----------|------|-----------|
| Оркестратор | `src/orchestrator/` | Реестр суб-агентов, политика спавна, лимиты глубины, уведомления |
| Контекст-движок | `src/context/` | Бюджет токенов, приоритетная сборка, автокомпакция |
| Canvas | `src/canvas/` | Интерактивный UI через Gateway (дашборды, формы, отчёты) |
| Персоны | `src/personas/` | Системные промпты, разрешённые инструменты, модельные предпочтения |
| Workspace | `src/workspace/` | Изоляция по пользователям, валидация путей, мульти-юзер |
| Delegate-инструмент | `src/tools/delegate.rs` | Мост между Sentinel и суб-агентами |
| Claude Code | `src/tools/claude_code.rs` | Планирование/реализация/ревью/фикс через CLI |

Подробности см. в основном английском README: [`README.md`](README.md)
