...
ZeroClaw 内置 **Sentinel**，一个多智能体编排器，协调专业化 AI 智能体完成复杂目标 — 从构建全栈产品到运营业务。
...
| 代号 | 角色 | 专长 |
|------|------|------|
| **Sentinel** | 编排器 | 分解目标、分配智能体、综合结果 |
| **Archon** | 系统架构师 | 系统设计、API 契约、架构决策 |
| **Prism** | UI/UX 专家 | 用户流程、组件、可访问性、设计系统 |
| **Forge** | 全栈工程师 | 实现、测试、调试、部署 |
| **Nexus** | 商务开发 | 合作关系、提案、战略 |
| **Echo** | 市场营销 | 内容、活动、定位、分析 |
| **Closer** | 销售 | 外联、资格审查、异议处理、演示 |
| **Oracle** | 研究员 | 市场分析、竞争情报、技术调研 |
| **Veasna** (វាសនា) | 高棉语专家 | 文化翻译、高棉语写作、本地化 |

### 工作流程

1. **用户发送目标** → Sentinel 分类并分解
2. **Sentinel 分配子任务** → 按专长路由到对应智能体
3. **智能体并行执行** → Builder、Business、Research 集群同时工作
4. **推送式完成** → 智能体完成后主动汇报（无需轮询）
5. **Sentinel 综合** → 将各输出合并为统一交付物

### 关键子系统

| 子系统 | 路径 | 功能 |
|--------|------|------|
| 编排器 | `src/orchestrator/` | 子智能体注册、生成策略、深度限制、完成通知 |
| 上下文引擎 | `src/context/` | Token 预算管理、优先级排序组装、自动压缩 |
| 画布 | `src/canvas/` | 通过 Gateway 渲染交互式 UI（仪表板、表单、报告） |
| 角色 | `src/personas/` | 每个智能体的系统提示、工具权限、模型偏好 |
| 工作区 | `src/workspace/` | 每用户隔离存储、路径验证、多用户管理 |
| 委托工具 | `src/tools/delegate.rs` | Sentinel 与子智能体之间的桥梁 |
| Claude Code | `src/tools/claude_code.rs` | 通过 CLI 进行计划/实现/审查/修复 |

详细信息请参阅英文主文档：[`README.md`](README.md)
